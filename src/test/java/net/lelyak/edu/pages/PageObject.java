...
import net.lelyak.edu.core.exceptions.NavigationPathNotFoundException;
import net.lelyak.edu.core.exceptions.PageNotFoundException;
import net.lelyak.edu.core.logging.Logger;
import net.lelyak.edu.pages.abibas.WelcomePage;
...
public abstract class PageObject implements INavigationPage {
        ...
        do {
            try {
                ...
                if (page.successor.containsKey(way.get(1))) {
                    ...
                    way.getFirst().newInstance().successor.get(way.get(1))
                            .click();
                }
            } catch (InstantiationException | IllegalAccessException e) {
                e.printStackTrace();
    ...
    private void showStatistic() {
        for (Map.Entry<List<Class<? extends PageObject>>, AtomicInteger> entry : pathReport
                .entrySet()) {
            Logger.logDebug(entry.getKey().toString() + " - "
                    + entry.getValue());
        }
    }
    ...
    public boolean isPageNameEquals(TextContainer e, String namePage) {
    ...
    public boolean waitMessiBox() throws TimeoutException {
        ...
        return wait.until(new Function<WebDriver, Boolean>() {
            public Boolean apply(WebDriver driver) {
                return getDriver().findElement(By.className("messi-box"))
                        .isDisplayed();
            }
        });
    }
